...
$("div.ui-dialog-content").bind("dialogclose", function(){ $('.main').find('object').show(); });
	...
	*/
	...
	// load first item -- this can also be done at init
	$.plopquiz.loadItem();
...
$("#quiz_timer", $.plopquiz.quiz_inner_content)
.bind("startTimer", function(event, quizItem)
{
	$timer = $("#quiz_timer", $.plopquiz.quiz_inner_content);
	var self = this;

	if(!quizItem || !quizItem.timed || $.plopquiz.settings.noTimer) return;
...
$.plopquiz.timer.animate({opacity: 1}, 2000, function() //temporarily pause the timer 
{
...
.bind("quizItemLoaded", function(e, quizItem)
{
	...
	$.plopquiz.quiz_inner_content.animate({opacity: 1},{ duration: 0, // delay start of timer 
			complete: function()
			{ 
